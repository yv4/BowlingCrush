...
///
/// To create and normalStart a Timer, use the <see cref="Register"/> method.
/// </summary>
public class Timer
{
    ...
    /// <summary>
    /// How long the timer takes to complete from normalStart to finish.
    /// </summary>
    ...
    /// of time passed in seconds since the normalStart of the timer's current loop.</param>
        ...
        bool isLooped = false, bool useRealTime = false, MonoBehaviour autoDestroyOwner = null)
    {
        ...
        if (Timer._manager == null)
        {
            ...
            else
            {
                ...
                managerObject.AddComponent<MobiiGame.Sdk.Base.DonotDestroyOnLoad>();
    ...
    /// <summary>
    /// Get how many seconds have elapsed since the normalStart of this timer's current cycle.
    /// </summary>
    /// <returns>The number of seconds that have elapsed since the normalStart of this timer's current cycle, i.e.
    /// the current loop if the timer is looped, or the normalStart if it isn't.
    ///
    ...
    /// <summary>
    /// Get how much progress the timer has made from normalStart to finish as a ratio.
    /// </summary>
    ...
    // for pausing, we push the normalStart time forward by the amount of time that has passed.
    // this will mess with the amount of time that elapsed when we're cancelled or paused if we just
    // check the normalStart time versus the current world time, so we need to cache the time that was elapsed
